...
} from '../../../lib/microsoft/rest/MicrosoftAuth';
import {
	MicrosoftErrorCode,
	MicrosoftResponse
} from '../../../lib/microsoft/rest/MicrosoftResponse';
import { assertResponse, expectSuccess } from '../../common/RestResponseUtil';

// eslint-disable-next-line @typescript-eslint/no-unused-vars
function expectMicrosoftResponse(
	res: MicrosoftResponse<unknown>,
	responseCode: MicrosoftErrorCode,
	negate = false
): void {
	assertResponse(res);
		...
		nock(XBL_AUTH_URL.origin)
			...
			.reply(
				...
				(
					uri,
					requestBody: unknown
				): { Identity: string; XErr: number; Message: string; Redirect: string } => {
					return {
						Identity: '0',
						XErr: 2148916238,
						Message: '',
...
describe('[Microsoft Auth] Auth', () => {
	it('getAccessToken (Auth + Refresh)', async () => {
		...
		nock(TOKEN_URL.origin)
			...
			.reply(
				200,
				(uri, requestBody: AuthTokenRequest | RefreshTokenRequest): AuthorizationTokenResponse => {
