...
import { ClientBoundPacket, ProtocolUtils, ServerBoundPacket } from './Protocol';
...
function getRequestPacket(): Buffer {
	return ServerBoundPacket.build().writeVarInt(0x00).toBuffer();
}
	...
	return new Promise((resolve, reject) => {
		...
		socket.once('data', (data) => {
			...
			if (packetType !== 0x00) {
				...
				socket.destroy();
				reject(
					new Error(`Invalid response. Expected packet type ${0x00}, received ${packetType}!`)
				);
				return;
			}
			...
			const packetReadListener = (nextData: Buffer, doAppend: boolean): void => {
				if (iterations > maxTries) {
					...
					reject(
						new Error(
							`Data read from ${hostname}:${port} exceeded ${maxTries} iterations, closing connection.`
						)
					);
					return;
				}
