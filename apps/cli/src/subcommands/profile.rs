...
impl ProfileAdd {
    pub async fn run(
        ...
        info!(
            "Adding profile at path '{}' to OneLauncher",
        ...
        let json_path = profile.join("profile.json");
            ...
            "Profile already managed by OneLauncher. If the contents of the profile are invalid or missing, the profile can be regenerated using `profile init` or `profile fetch`"
        );
...
/// create a new profile and manage it with OneLauncher
...
impl ProfileRemove {
    pub async fn run(
        ...
        let profile = self.profile.canonicalize()?;
        info!("Removing profile {} from OneLauncher", self.profile.display());
...
impl ProfileRun {
    pub async fn run(
        ...
        let path = self.profile.canonicalize()?;
           ...
           "Profile not managed by OneLauncher (if it exists, try using `profile add` first!)",
        );
