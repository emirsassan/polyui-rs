use crate::{
	invalidate_query, node::Platform, prisma::node, utils::db::load_and_migrate, NodeContext,
};
...
impl LibraryManager {
	...
	pub(crate) async fn create(
		...
		let id = Uuid::new_v4();
		LibraryConfig::save(
			Path::new(&self.libraries_dir).join(format!("{id}.polyuilibrary")),
			&config,
		)
		.await?;
	...
	pub async fn delete_library(&self, id: Uuid) -> Result<(), LibraryManagerError> {
		...
		fs::remove_file(
			Path::new(&self.libraries_dir).join(format!("{}.polyuilibrary", library.id)),
		)?;
