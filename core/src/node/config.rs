...
pub const NODE_STATE_CONFIG_NAME: &str = "node_state.puiconfig";
...
impl NodeConfigManager {
	...
	fn migrate_config(
		...
		match current_version {
			None => {
				Err(NodeConfigError::Migration(format!("Your Polyfrost config file stored at '{}' is missing the `version` field. If you just upgraded please delete the file and restart the app!", config_path.display())))
			}
			_ => Ok(()),
		}
	}
