...
) -> crate::error::Result<GameVersionInfo> {
...
pub async fn download_assets_index(
    st: &State,
    version: &GameVersionInfo,
) -> crate::error::Result<AssetsIndex> {
...
pub async fn download_assets(
    ...
    stream::iter(index.objects.iter())
        .map(Ok::<(&String, &Asset), crate::error::Error>)
...
pub async fn download_libraries(
    ...
    stream::iter(libraries.iter())
        .map(Ok::<&Library, crate::error::Error>)
                ...
                async {
                    if let Some((os_key, classifiers)) = None.or_else(|| Some((
                        ...
                        let parsed_key = os_key.replace(
                            "${arch}",
                            crate::utils::platform::ARCH_WIDTH,
                        );
