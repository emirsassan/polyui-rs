use crate::{state as st, wrap_ref_builder};
use daedalus as d;
...
pub fn parse_rule(rule: &d::minecraft::Rule) -> bool {
    ...
    let res = match rule {
        Rule {
            os: Some(ref os), ..
        } => crate::utils::platform::os_rule(os),
        Rule {
...
pub async fn launch_minecraft(
    ...
    let version = state
        ...
        .ok_or(crate::error::CoreErrors::LauncherError(format!(
                    ...
                    .arg(
                        ...
                        .ok_or_else(|| {
                            crate::error::CoreErrors::LauncherError(format!(
                                "Could not processor main class for {}",
                                processor.jar
                            ))
                        })?,
                    )
